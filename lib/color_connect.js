export class Board {
  ...
  validPathCreated(color, startPos, endPos) {
  ...
  won() {
    ...
    for (let i = 0; i < colors.length; i++) {
      ...
      if (!this.validPathCreated(color, startPos, endPos)) {
        return false;
      }
    }
...
export class Tile {
  ...
  isNeighbor(otherTilePos) {
    return (
      ((otherTilePos[0] - this.pos[0] === 0) && Math.abs(otherTilePos[1] - this.pos[1]) === 1) ||
      ((otherTilePos[1] - this.pos[1] === 0) && Math.abs(otherTilePos[0] - this.pos[0]) === 1)
    );
  }
}
